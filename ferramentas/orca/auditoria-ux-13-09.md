...
- (59) Trabalhos: a linha dizia "13 de set. de 2026" (formato de máquina); agora "ontem", "há 2 dias", como as Notas (`VozDoAutor.relativo`). Captura `trabalhos-data-relativa-14-09.png`. Suíte 1208/183 verde.
- (60) Lente: "Apontar um trecho" (campo livre + quatro rótulos para a pessoa classificar) saiu — a Lente já acha muletas, frases feitas, passivas e advérbios sozinha, e cada achado leva os rótulos na própria linha; a pessoa não classifica taxonomia. Captura `lente-sem-apontar-14-09.png` (antes em `lente-depois-14-09.png`). Suíte 1208/183 verde.
...
- (65) Recordar filmado (`recordar-cerimonia-14-09.mp4`: revelar em 12 quadros, botão em 6) — e, no lado a lado, "a nota" saía com os campos embaralhados ("plano, obstáculo, resultado"): `VozDoAutor.juntar` lia `campos.values` na ordem do dicionário. Agora `respostasNaOrdemDoMetodo` (o método que tem esses campos dá a ordem; sem método, a chave) — vale para a voz do autor em toda parte (classificação, corpus, Recordar). Teste `OrdemDosCamposTests`. Capturas `recordar-ordem-antes-14-09.png` / `recordar-ordem-depois-14-09.png`. Suíte 1210/184 verde.
- (66) Pedido pelo campo, segunda parte: o verbo do pedido não é o título. "me lembra de ligar para a Ana amanha 15h" (sem acento, como se digita) marcou "Ligar para a Ana" em 15/09 às 15h — provado pelo disco do Air (`calendario.json`), não pela tela. `CalendarioFrase.limparTitulo` tira "me lembra de / marca / agenda / anota / coloca…" do começo. Teste `PedidoNoCalendarioTests`. Captura `pedido-lembra-campo-14-09.png`. Suíte 1213/185 verde.
- (67) O aviso da Sessão ("Cabelo marcado para sábado 19, 10:00") só era desenhado na página: nas Notas o pedido marcava e a tela calava — a pessoa não sabia se tinha dado certo. O aviso passou a pousar sobre o campo em todas as telas do arquivo (RaizView, o mesmo cartão da página). Provado com "marca cabelo sabado 10h" (captura `toast-notas-14-09.png`; o compromisso no `calendario.json`). Suíte 1213/185 verde. De caminho: o domínio da nota "Quero dormir mais cedo" virou Saúde sozinho (o modelo de bordo reclassificou) — a IA agindo em silêncio, como o goal pede.
- (68) Notas: "nenhuma nota com “…”." em letra de corpo era manchete por cima dos achados pelo sentido; com achados abaixo, vira nota de rodapé (meta, tinta fraca); sem nenhum achado continua em corpo. Capturas `busca-vazio-antes-14-09.png` / `busca-vazio-depois-14-09.png`. Suíte 1213/185 verde.
- (69) Notas: a porta "1 trabalho ›" aparecia dentro do resultado de uma busca. Só existe com o campo vazio. Captura `busca-sem-porta-14-09.png`. Suíte 1213/185 verde.

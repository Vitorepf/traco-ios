...
## A colagem, feita no mesmo dia

Depois desta conferência a leva 3 foi colada, na ordem dos cinco passos do
pacote. O que entrou:

| passo | o que foi feito |
|---|---|
| 1 | A Inversão saiu do `Metodos.json`; o Pré-mortem ganhou a frase de abertura ("como você GARANTIRIA que isto falhasse?"), **herdou a linha de regex** e recebeu o `recordar` que a fusão custaria |
| 2 | A reversibilidade saiu do `movimento` da Decisão — a Porta entrou e o catálogo não diz a mesma coisa em dois lugares |
| 3 | **Nada: já estava em `main`.** As duas frases de origem (Decisão e Primeiros princípios) foram aplicadas numa volta anterior. A 3.3 (`notaPermanente`) ficou de fora: citar *Kommunikation mit Zettelkästen* sem ter lido seria a régua da proveniência sendo quebrada pela própria trilha |
| 4 | As seis frases de voz trocadas. **A sétima não** (`praticaDeliberada`): o pacote a deixa a critério do dono, e ela continua a critério dele |
| 5 | Os catorze no fim do array, na ordem do pacote. **41 métodos** |

### Os onze lugares, consertados

Os dez da tabela acima, mais um que apareceu na hora:
`maestro/metodos-m3.yaml` afirma `"28 do app"` na tela do Perfil — passou a
`"41 do app"`. O pacote não o lista, e ele nasceu na colagem da leva 1.

`maestro/ilhas-encadeadas.yaml` não foi remendado: o assunto dele era a cadeia
Inversão → Pré-mortem, que a fusão apaga. Foi **reescrito para Porta → Decisão**,
que é da mesma leva, tem a mesma forma (dois campos, um botão, os campos
copiados no destino) e prova a mesma lei da ADR 04z.

### O que não foi feito, e é decisão do dono

- **A sétima frase de voz** (`praticaDeliberada`), pelo motivo do próprio pacote.
- **O gatilho `magoei` da Reparação** continua no `roteamento`. Ele nunca vai
  disparar, e há duas leituras: tirar (gatilho que não dispara é dívida
  escondida) ou manter como intenção declarada, já que a guarda o cobre. Ficou
  **declarado no teste**, não escondido — ver abaixo.
- **O bloco `aposentados`** do §1.6b: uma nota antiga de Inversão mostra o id cru
  onde antes mostrava "Inversão". O pacote não o propõe e esta colagem também
  não.

### Os quatro ramos que entram em `conhecidos`

O `todoRamoDeRegexAlcancaOSeuMetodo` expande cada ramo de cada regex em sondas e
exige que cada uma chegue no seu próprio método, com uma lista de exceções
declaradas. Rodando o mesmo expansor em Python sobre os 41: **545 sondas, quatro
exceções novas** — e as quatro estão escritas no teste, com o motivo:

| ramo | chega em | por quê |
|---|---|---|
| `fatoContrario` · "derruba a minha ideia" | `notaPermanente` | o desvio herdado II.1: "ideia" da Nota permanente come regex específica. Pré-existente, medido, com volta própria |
| `fatoContrario` · "derruba minha ideia" | `notaPermanente` | idem |
| `reparacao` · "magoei" | silêncio | a guarda (`mago[aeiou]` em `lexicoDoSentimento`) |
| `porta` · "se eu me arrepender" | silêncio | a guarda (`arrepend`, o mesmo léxico) |

### O que só o Mac roda

Esta sessão **não compila e não roda a suíte**. O que precisa ser rodado, na
ordem:

1. **`swift test`** (ou o esquema `Traco` no Xcode). Os testes tocados foram
   `CatalogoTests` (contagem, lista, sufixos, o par de encadeamento, a frase do
   método ausente, o destino da regex herdada, os quatro ramos) e
   `EscritaPessoalTests` (três frases que passam da Inversão ao Pré-mortem).
   **O que eu não pude verificar: que tudo isso compila.** As mudanças de Swift
   são literais de string, comentários e asserções; o risco é baixo e não é zero.
2. **`python3 ferramentas/orca/metodos/conferir-leva-3.py`** — segundos, sem
   build. Ele detecta sozinho que a colagem já aconteceu e confere o catálogo do
   disco. Verde aqui em 17/09: 41 métodos, os catorze presentes, a herança no
   lugar, zero encadeamento morto, a guarda segurando o desabafo longo, alcance
   14/14 e as 121 frases dos testes indo para onde o teste declara.
3. **Os dois fluxos de maestro reescritos**: `metodos-m3.yaml` (o "41 do app" na
   tela) e `ilhas-encadeadas.yaml` (a cadeia Porta → Decisão). **Nenhum dos dois
   foi executado.**
4. **A Lente de uma nota antiga de Inversão**, mostrando `metodo-ausente` com os
   campos preservados e a frase nova. É a prova de que o §1.6 é verdade na tela.
5. **A análise de bordo com 41**, que o pacote pedia contra "os dez" do enum
   morto — esse teto já caiu (ADR 06g), e o que resta é ver o modelo do aparelho
   escolhendo entre 41.

# Leva 3 e a fusão — o pacote de execução

Trilha Métodos · volta M15 · 06/09/2026. **Este arquivo é tudo que a trilha
produziu e que ainda não está no app.** Quem for executar não precisa abrir
nenhuma ficha: o JSON está aqui, as frases de conserto estão aqui, as provas
estão aqui.

**Base:** este pacote assume a **leva 1 mesclada** (os sete da M3 —
`Vitorepf/volta-m3-colagem`, catálogo com 28). Se ela ainda não estiver em `main`,
mescle primeiro: três encadeamentos deste pacote apontam para métodos da leva 1
e nasceriam mortos.

**Resultado no fim:** **41 métodos** — 28 da base, menos a Inversão, mais os 14
deste pacote.

## A ordem dos cinco passos (e ela importa)

| # | passo | toca | por que nesta ordem |
|---|---|---|---|
| 1 | **A fusão da Inversão no Pré-mortem** | JSON + 2 testes | é deleção; feita antes, o array já entra limpo |
| 2 | **Higiene II.0: a reversibilidade sai da Decisão** | 1 frase | tem de acontecer na mesma volta em que a Porta entra, senão o catálogo diz a mesma coisa em dois lugares |
| 3 | **Duas frases de origem (M6)** | 2 frases | independente; barato |
| 4 | **Seis (ou sete) frases de voz (M8)** | 6–7 frases | independente; barato |
| 5 | **A leva 2 + 3: os 14, num array só** | fim do array | por último, porque só depois de 1 o array tem o tamanho final |

---

# PASSO 1 — A fusão da Inversão no Pré-mortem

Decisão do orquestrador na M15: **sai a Inversão, e só ela.** Mesmo movimento do
Pré-mortem, que faz mais (a falha enquadrada como já ocorrida, o sinal precoce, a
data de vigiar), e com a proveniência mais fraca dos 21 pela auditoria da M6
("discursos (1986 em diante)" não localiza nada).

## 1.1 O que sai do JSON

O objeto inteiro de `"id": "inversao"` em `Traco/Modelo/Metodos.json` — hoje na
**posição 16** do array, entre `analogia` e `steelman`. **Nada mais sai:** nenhum
outro método do catálogo tem encadeamento apontando para ele (conferido nos 28 da
base e nos 14 deste pacote: zero referências).

## 1.2 A frase que entra no movimento do Pré-mortem

O movimento do Pré-mortem hoje:

> "Pré-mortem (Klein, 2007). O enquadramento é obrigatório: é um ano depois e o
> plano JÁ FALHOU — nunca "o que pode dar errado". Cobre as causas dessa falha,
> qual é a mais provável, e que sinal PRECOCE apareceria primeiro."

**Com a fusão** (a pergunta da Inversão vira a abertura, e o resto fica igual):

> "Pré-mortem (Klein, 2007). O enquadramento é obrigatório: é um ano depois e o
> plano JÁ FALHOU — nunca "o que pode dar errado". Se ajudar a começar, o
> caminho curto é o contrário: como você GARANTIRIA que isto falhasse? Cobre as
> causas dessa falha, qual é a mais provável, e que sinal PRECOCE apareceria
> primeiro."

Uma frase, no meio. **Nenhum campo novo** — os quatro do Pré-mortem (`plano`,
`falhou`, `sinal`, `mudo`) continuam iguais, e nenhuma nota existente muda.

Repare que a frase é uma **oferta de caminho**, não uma sentença sobre quem
escreve: é o modelo da régua da voz, e o motivo de ela dizer "se ajudar a
começar" em vez de "todo mundo trava aqui".

## 1.3 A linha de regex que o Pré-mortem herda — **e por que não se pode só apagar**

Este é o achado do passo. Apagar a Inversão sem mais nada **deixa cinco frases de
gatilho sem dono**: hoje elas abrem uma forma, depois abririam nada.

| frase | hoje | só apagando | com a herança |
|---|---|---|---|
| "o que evitar aqui" | Inversão | **ninguém** | Pré-mortem |
| "pensar ao contrário" | Inversão | **ninguém** | Pré-mortem |
| "o pior jeito de fazer isso" | Inversão | **ninguém** | Pré-mortem |
| "inversão" | Inversão | **ninguém** | Pré-mortem |
| "como garantir que isso falhe" | ninguém¹ | ninguém | Pré-mortem |

¹ Já não achava nada hoje: a regex da Inversão é `como garantir que (falhe|dê
errado)`, e "que **isso** falhe" não casa. A linha herdada conserta isso de
graça.

**A linha a acrescentar ao `roteamento` do Pré-mortem** (como último elemento do
array, depois da que ele já tem):

```
\binvers[ãa]o\b|\bcomo garantir que (isso |ele |ela |o plano )?(falhe|d[êe] errado)\b|\bo que evitar\b|\bpior jeito de\b|\bao contr[áa]rio\b
```

**Prova rodada:** com esta linha, o Pré-mortem sobe da posição 16 (onde a
Inversão estava) para a 10 (onde ele está) na disputa por essas frases. Testei 26
frases contra o catálogo antes (28 métodos) e depois (41 com a fusão): **a única
mudança fora da Inversão é uma melhora** — "como garantir que isso falhe", que
não achava ninguém, passa a achar o Pré-mortem. **Zero regressão.**

## 1.4 O que a fusão custa, e não dá para esconder

**O `recordar` da Inversão morre.** Ela tem um (alvo `falhe`+`evito`, pista
`quero`); o Pré-mortem **não tem**. Sem conserto, o Recordar de uma nota de
Pré-mortem cai em `.livre` — `RecordarView.swift:17`, `if let spec =
gesto?.metodoDef.recordar … return .livre`. Não quebra nada; só deixa de ser
memória dirigida.

**O conserto, se o dono quiser** (opcional, e o único acréscimo desta volta):

```json
"recordar": {
  "alvo": ["falhou", "sinal"],
  "pista": ["plano"],
  "pergunta": "Um ano depois, o que tinha acontecido — e qual foi o primeiro sinal?",
  "instrucao": "O plano fica. A falha some.",
  "rotuloAlvo": "A FALHA"
}
```

Entra no objeto do Pré-mortem, entre `campos` e `encadeamentos`. Se o dono
preferir não mexer, **diga isso na ADR**: a fusão passa a custar um `recordar`, e
custo declarado não é dívida escondida.

## 1.5 Os dois testes que quebram — conferidos, com linha

Em `TracoTests/CatalogoTests.swift` (de `main`):

- **linha 10**, `oBundleTemOsVinteEUmMetodos`: `#expect(ids.count == 21)` e a
  lista literal que inclui `"inversao"` (linha 13). Vira **20** sem ela — ou o
  número final do catálogo, se a leva entrar na mesma volta.
- **linha 163**: `#expect(Catalogo.metodo("inversao")?.encadeamentos.first?.para == "premortem")`.
  Com a Inversão fora, `Catalogo.metodo` devolve `nil` e a comparação falha.
  **Trocar por outro par vivo** — por exemplo
  `Catalogo.metodo("porta")?.encadeamentos.first?.para == "decisao"`.

Nada mais em `Traco/` cita `inversao`: conferido em `main`, só estas duas linhas
de teste e o próprio JSON.

## 1.6 **A nota que já usa a forma Inversão** — a pergunta do dono

**A resposta é sim: a nota conserva a forma antiga, e o catálogo diz que ela
saiu. A lei já existe, já está escrita e já está testada** — a volta 16 a
construiu justamente para isto. Onde ela mora:

1. **`Traco/Modelo/Gesto.swift:9-19` — o `Gesto` é uma `String`, não um `case`.**
   `init?(rawValue:)` aceita qualquer id não vazio. O comentário do arquivo é
   explícito: *"Todo id conhecido pelo catálogo, e também os que já foram
   gravados num `gestoRaw` e cujo método sumiu da pasta: a nota não perde o
   gesto."* A nota no disco guarda `"inversao"` e continua guardando.

2. **`Gesto.swift:56` — `metodoDef`** cai em `Metodo.desconhecido(rawValue)`
   (`Metodo.swift:143`), que devolve um método de nome igual ao id e sem campos.
   **Os valores que o autor escreveu não estão no método: estão na nota.** O
   `Metodo` só diz como desenhar.

3. **`Gesto.swift:66-69` — `estadoDoMetodo` (ADR 05x)**, a linha que a tela
   mostra, com o comentário que é a própria lei: *"Estado honesto, não bloqueio:
   os campos ficam."*

4. **`Traco/Pagina/LenteView.swift:67`** desenha essa linha na Lente, com
   `accessibilityIdentifier("metodo-ausente")`.

5. **`TracoTests/CatalogoTests.swift:224`** já prova a frase exata para um id
   inventado.

6. **`Traco/Notas/Corpus.swift:222-224` (ADR 05o)** garante o mesmo na exportação:
   *"quando o método saiu da pasta, as respostas do autor não podem sair junto"* —
   quem delimita o bloco de campos no `.md` é o marcador, não o rótulo do
   catálogo.

**Portanto a fusão não pede nenhuma migração de dado, nenhum aviso, nenhuma
conversão.** É o caso para o qual a lei foi escrita.

### Mas a lei tem duas arestas, e elas são desta volta

**(a) A frase mente sobre quem tirou.** `estadoDoMetodo` diz:

> o método "inversao" **saiu da sua pasta**; os campos continuam na nota.

Isso é verdade quando o autor apagou um método da pasta dele. **Aqui é falso: o
autor nunca teve a Inversão numa pasta — ela veio no app, e quem a tirou fomos
nós.** Pela régua da voz, é o app afirmando um fato sobre uma ação do autor que
não aconteceu. E não dá para distinguir os dois casos depois do fato, porque o
método sumiu junto com a informação de onde ele vinha.

**Conserto, uma linha, e serve aos dois casos:**

> o método "inversao" **não está mais no catálogo**; os campos continuam na nota.

Muda `Gesto.swift:68` e a string esperada em `CatalogoTests.swift:224`. **Se a
volta não quiser mexer em Swift, então a fusão não deve acontecer nesta volta** —
melhor adiar uma deleção do que publicar uma frase falsa para o autor.

**(b) O nome vira o id.** Com o método fora, `gesto.nome` é `metodoDef.nome`, que
`desconhecido` define como o **id cru**. O autor lê `inversao`, em minúscula e sem
acento, onde antes lia **Inversão** — um nome que ele nunca viu na vida. É
cosmético e é honesto, mas é feio.

Duas saídas, e nenhuma é obrigatória:
- **Não fazer nada.** Uma nota antiga mostra o id. Aceitável, e é o que a lei já
  faz hoje para método apagado pelo autor.
- **Um bloco `aposentados` no `Metodos.json`** (`{"inversao": "Inversão"}`) que
  `desconhecido` consulta para o nome. É código, é pequeno, e **eu não o proponho
  nesta volta**: resolve um caso que só vai acontecer de novo se o catálogo
  aposentar outro método, e a régua da M14 diz que isso vai ser raro.

---

# PASSO 2 — Higiene II.0: a reversibilidade sai da Decisão

**Só se a Porta entrar** (ela está no array do passo 5). O catálogo não deve
dizer a mesma coisa em dois lugares, e o lugar novo é uma forma com campos, não
uma frase de prosa.

No `movimento` da **Decisão**, sai:

> "quanto custa errar para cada lado, e se dá para desfazer. Decisão reversível e
> barata não merece o mesmo cuidado que uma que não volta."

**Não substituir por nada.** O que sobra do movimento da Decisão continua
completo (opções, critério, o decidido, o que se espera com data), e quem quiser
a triagem tem a Porta — que, aliás, encadeia PARA a Decisão.

---

# PASSO 3 — As duas frases de origem (auditoria da M6)

Eram três; a terceira era a Inversão, que sai no passo 1. Ambas são o mesmo
defeito: **grau declarado acima do real.** Nenhuma mente uma palavra; as duas
citam obra real ao lado de um método que não está nela.

### 3.1 `decisao` → `proveniencia.fonte`
...
> descreva. Kahneman e Klein, Conditions for Intuitive Expertise (2009), tratam de
> quando a intuição de especialista é confiável — é evidência vizinha sobre
> julgamento, não a origem deste método."

*Fui ao artigo (American Psychologist 64, p. 515–526, doi:10.1037/a0016755): não
há diário de decisão nele.*

### 3.2 `primeirosPrincipios` → `proveniencia.adaptacao`

**Hoje:**
> "O que acho que sei, o que é verdade de fato, o que construo do zero."

**Trocar por:**
> "Aristóteles não propõe este exercício; o Traço toma dele a noção de princípio e
> monta o resto: o que acho que sei, o que é verdade de fato, o que construo do
> zero."

*A `fonte` pode ficar como está — usar Aristóteles como lente é legítimo, e
`funcao: lente` já está certo. O que faltava era separar a lente do procedimento.*

### 3.3 Opcional, e barata: `notaPermanente`

A ficha diz que a prática "é atestada pela obra de Luhmann", o que é verdade.
Citar o texto em que ele descreve o próprio Zettelkasten
(*"Kommunikation mit Zettelkästen"*, 1981) elevaria a ficha a um grau A limpo.
Não é defeito; é melhoria.

---

# PASSO 4 — As seis frases de voz (auditoria da M8)

Todas as seis **ainda estão em `main`**, conferidas hoje. O defeito é um só: o
`movimento` afirma um fato sobre as pessoas que ninguém mediu, na voz do app.
O conserto é sempre o mesmo — **trocar a afirmação sobre as pessoas pela cobrança
do método** —, e o modelo certo já existe dentro do próprio catálogo.

| id | sai | entra |
|---|---|---|
| `woop` | "…e é o passo que todo mundo pula escrevendo obstáculo externo." | "Obstáculo externo (o relógio, os outros) não conta, e o método recusa." |
| `argumento` | "O passo que se pula é a objeção forte: todo mundo responde à fraca." | "O passo que o método cobra é a objeção FORTE — não a que é fácil de derrubar." |
| `dia` | "O passo que se pula é nomear o ladrão antes: todo mundo planeja como se o dia fosse obedecer." | "O passo que o método cobra é nomear o ladrão ANTES, e não depois que o dia acabou." |
| `divergencia` | "O passo que se pula é chegar a dez: a maioria para na terceira, que é a óbvia." | "O passo que o método cobra é chegar a dez, e a conta é literal: com três, não houve divergência." |
| `leitura` | "O passo que se pula é discordar: quem só concorda não leu, copiou." | "O passo que o método cobra é a discordância: sem um ponto de discordância escrito, a tese ficou nas palavras do livro, não nas suas." |
| `feynman` | "O passo que se pula é o travar: quem explica sem travar usou jargão no lugar da compreensão." | "O passo que o método cobra é o travar: se a explicação saiu inteira, o método pede a palavra que você usou sem saber explicar." |

**A sétima, opcional** — `praticaDeliberada`: "treinar a habilidade inteira é
repetir o que já se faz bem" → "o exercício tem de ser MENOR que a habilidade,
senão ele não ataca o que falha". É a mais defensável das sete (é a tese central
de Ericsson, e a ficha cita Ericsson), e por isso fica a critério do dono.

**A do Feynman não é só de voz: é falsa como regra.** Quem entende de verdade
também explica sem travar. Se só uma das sete entrar, que seja essa.

O raciocínio inteiro, com o "por quê" de cada uma, está em
[`auditoria-eficacia.md`](auditoria-eficacia.md). **Dois achados do mesmo tipo
eram meus** (Coluna da esquerda e O ponto que decide) e já vêm corrigidos no
array do passo 5.

---

# PASSO 5 — A leva 2 e a leva 3: os catorze, num array só

**Onde colar: no FIM do array**, nesta ordem, sem reescrever nada.

**Por que no fim, e isto é lei da trilha:** nenhum método cuja regex mencione
conversa, silêncio, arrependimento ou sentimento pode entrar **antes da
Expressiva**. Está medido desde a M1 — com a Coluna da esquerda antes dela, a
frase de desabafo do teste vira formulário. **A proteção da escrita pessoal, aqui,
é a ordem do arquivo; não há guarda em código que salve se a ordem mudar.**
Vale para a Reparação e para o Exame da noite tanto quanto para a Coluna.

**Cole os catorze juntos ou tire os encadeamentos internos.** Dois apontam para
dentro da própria leva — `oQueNaoEsta → combinado` e `sobrevivente →
transferencia` —, e cinco apontam para a leva 1 (`ordemDeGrandeza →
classeDeReferencia`, `comecariaHoje → subtracao`, `estaBom → subtracao`,
`regraQueEuFaco → vistoNaoVisto`, `combinado → colunaEsquerda`). Colar menos do
que está aqui exige tirar o que ficou apontando para fora — senão nasce **botão
morto**: `Sessao.encadear:753` sai em silêncio e `CamposFormaView` desenha o botão
assim mesmo.

## Os catorze, com grau e capacidade

| # | id | nome | grau | capacidade | campos |
|---|---|---|---|---|---|
| 1 | `fatoContrario` | A nota do fato contrário | A | Pensar com rigor | 5 |
| 2 | `ordemDeGrandeza` | Ordem de grandeza | A | Decidir e apostar | 6 |
| 3 | `comecariaHoje` | Começaria hoje? | A | Decidir e apostar | 6 |
| 4 | `combinado` | O combinado | A | Conviver | 6 |
| 5 | `pontoQueDecide` | O ponto que decide | **C** | Pensar com rigor | 7 |
| 6 | `oQueSeRepetiu` | O que se repetiu | A | Conduzir-se | 6 |
| 7 | `regraQueEuFaco` | A regra que eu faço | A | Conduzir-se | 6 |
| 8 | `reparacao` | A reparação | A | Conviver | 6 |
| 9 | `verAntesDeNomear` | Ver antes de nomear | A | Perceber | 5 |
| 10 | `oQueNaoEsta` | O que não está lá | A (ficção) | Perceber | 5 |
| 11 | `estaBom` | Está bom? | A | Construir e fazer | 8 |
| 12 | `porta` | Porta | A | Decidir e apostar | 4 |
| 13 | `transferencia` | Transferência | A | Aprender e transferir | 6 |
| 14 | `sobrevivente` | Sobrevivente | A | Pensar com rigor | 6 |

**Dois graus para ler com atenção:**

- **`pontoQueDecide` é grau C** — post de comunidade, sem editora e sem revisão de
  pares, e o próprio texto diz que o algoritmo "tem alguns buracos e esquisitices".
  A ficha declara isso, que é o que a régua exige. Ele é também o único dos 41 que
  a M14 deixou **em observação por sobreposição** (o campo `dele` é o Steelman).
- **`oQueNaoEsta` é ficção** — Sherlock Holmes. Pela emenda da M10, ficção é grau A
  de forma e o mais fraco de conteúdo, e obriga a linha que está lá: *"É ficção do
  século XIX. Não estabelece nada."*

## O array, pronto para colar

Estes são os **objetos**, para entrar no fim do array de `metodos` —
sem os colchetes externos, e com a vírgula que os liga ao último método que já
está lá.
...
},
{
  "id": "estaBom",
  "nome": "Está bom?",
  "origem": "Horácio, Ars Poetica",
  "faculdade": "criação",
  "proveniencia": {
    "fonte": "GRAU A: obra publicada, lida no texto integral em tradução identificada. Horácio, Ars Poetica (c. 19 a.C.), versos 385–390 e 438–452; tradução inglesa em prosa de The Works of Horace, Project Gutenberg, eBook 14020. A frase do enfeite tem segunda fonte, também lida: Arthur Quiller-Couch, On the Art of Writing (1916), aula XII, \"On Style\".",
    "funcao": "lente",
    "adaptacao": "Horácio dá três coisas e o Traço as põe em campos: guardar antes de julgar (\"até o nono ano\"), mostrar a um juiz competente, e cortar o enfeite. O nono ano vira uma data que o autor escolhe. Os dois campos que são do Traço são os que este autor precisa: onde ele DEFENDEU em vez de corrigir, e a pergunta que ninguém faz — é ruim, ou eu estou cansado?",
    "evidencia": "É um poema didático de dois mil anos, uma lente: não há estudo, não há medida, e não pode haver. Horácio fala de versos e de leitura em público; nada nele trata de produto, de código ou de trabalho delegado, e a transposição é do Traço. Não há evidência de que guardar por sete dias ou perguntar a um juiz melhore o que quer que seja — o que há é a experiência de dois milênios de gente que faz coisas, o que a régua chama de lente, e não de prova.",
    "aplicabilidade": "Serve para uma coisa feita, pronta ou quase, que o autor está julgando sozinho. Não serve antes de existir — aí é a Especificação, que define o pronto — nem para a coisa de outra pessoa."
  },
  "filtro": "Está bom?",
  "reconhecimento": "isto é uma coisa feita, e você está julgando sozinho.",
  "movimento": "Ars Poetica (Horácio). Três movimentos e mais dois que o Traço acrescenta. De Horácio: guardar antes de julgar — \"que seja suprimido até o nono ano\" —, mostrar a um juiz competente, e cortar o enfeite: o bom crítico \"poda os ornamentos ambiciosos\". Do Traço: onde você DEFENDEU em vez de corrigir, que é o momento exato em que o julgamento acaba; e a pergunta que ninguém faz — isto é \"não está bom\" ou eu estou cansado? Cobre a leitura mais dura da promessa que a coisa faz, e a decisão: mexer mais, ou ir assim com os defeitos aceitos escritos.",
  "pergunta": "Isto é \"não está bom\", ou você está cansado? Como você sabe?",
  "roteamento": [
    "\\best[áa] bom (assim|o suficiente|\\?)|\\bser[áa] que (est[áa]|t[áa]) bom\\b|\\bt[áa] bom assim\\b",
    "\\bquando (eu )?paro de mexer\\b|\\bfico mexendo\\b|\\bn[ãa]o sei se (paro|mexo mais|continuo mexendo)\\b",
    "\\bdou por (pronto|encerrado|acabado)\\b|\\bmando assim mesmo\\b|\\bacho que acabei\\b"
  ],
  "campos": [
    {
      "id": "oQue",
      "rotulo": "O que eu fiz, e o que isso prometia fazer"
    },
    {
      "id": "falha",
      "rotulo": "Onde NÃO cumpre a promessa — na leitura mais dura"
    },
    {
      "id": "enfeite",
      "rotulo": "A parte de que eu mais gosto: ela serve à promessa, ou a mim?"
    },
    {
      "id": "juiz",
      "rotulo": "Quem pode julgar isto e não sou eu — e o que eu pergunto a ele"
    },
    {
      "id": "defendi",
      "rotulo": "Onde eu defendi em vez de corrigir"
    },
    {
      "id": "cansado",
      "rotulo": "É \"não está bom\" ou eu estou cansado? Como eu sei"
    },
    {
      "id": "decido",
      "rotulo": "Mexo mais, ou vai assim — com estes defeitos aceitos"
    },
    {
      "id": "distancia",
      "rotulo": "Olhando de novo, com distância: o que eu vejo agora",
      "soDepois": true
    }
  ],
  "recordar": {
    ...
    "pergunta": "Onde não cumpria a promessa — e qual era a parte de que você mais gostava?",
    "instrucao": "O que você fez fica. O julgamento some.",
    "rotuloAlvo": "O JULGAMENTO"
  },
  "encadeamentos": [
    {
      "rotulo": "Tirar o enfeite (Subtração)",
      "para": "subtracao",
      "mapa": {
        "melhorar": "oQue",
        "sai": "enfeite"
    ...
    {
      "rotulo": "Olhar de novo em 7 dias",
      "compromisso": {
        "titulo": "olhar de novo: ",
        "campo": "oQue",
        "dias": 7
  ...
  "definicao": "algo já feito, esperando o julgamento do próprio autor (\"está bom?\", \"quando eu paro de mexer\")"
},
{
  "id": "porta",
  "nome": "Porta",
...
## Encadeamentos: o que nasce vivo e o que ainda espera

**Vivos assim que os catorze entrarem** (29 encadeamentos ao todo; todos com
destino conferido contra o catálogo final de 41):

| de | para | mora em |
|---|---|---|
| `fatoContrario` | `atualizacao`, `argumento` | os 21 |
| `ordemDeGrandeza` | `classeDeReferencia` | leva 1 |
| `comecariaHoje` | `decisao`, `subtracao` | 21 / leva 1 |
| `combinado` | `colunaEsquerda` | leva 1 |
| `pontoQueDecide` | `steelman`, `atualizacao` | os 21 |
| `oQueSeRepetiu` | `seEntao` | os 21 |
| `regraQueEuFaco` | `decisao`, `vistoNaoVisto` | 21 / leva 1 |
| `reparacao` | `seEntao` | os 21 |
| `verAntesDeNomear` | `argumento` | os 21 |
| `oQueNaoEsta` | `combinado` | **dentro desta leva** |
| `estaBom` | `subtracao` | leva 1 |
| `porta` | `decisao`, `premortem` | os 21 |
| `transferencia` | `atualizacao` | os 21 |
| `sobrevivente` | `transferencia`, `argumento` | **um dentro desta leva** |

Mais **nove compromissos com data** (que não dependem de destino nenhum:
Ordem de grandeza 30 dias, Começaria hoje? 30, O combinado 7, O que se repetiu 90,
A reparação 3, Ver antes de nomear 3, O que não está lá 3, Está bom? 7,
Transferência 14).

**Os que ainda esperam o app, e por isso NÃO estão no JSON** (o método funciona
achatado; a forma cheia é volta de código):

- **Campo repetível** (achado III.1): Cinco porquês, Divergência, Classe de
  referência e Ordem de grandeza pedem cadeia de profundidade variável. Hoje são
  campos fixos, e o Cinco porquês fica com nove.
- **Compromisso recorrente** (III.2): O que se repetiu e Prática deliberada
  pedem ritmo, não uma data só.
- **Duas colunas emparelhadas** (III.3): a Coluna da esquerda é isso, e hoje ela
  é dois campos soltos.
- **A Classe de referência lendo o corpus** (III.4): o pedido grande, e o que
  destravaria O que se repetiu — hoje a contagem é de memória, que é o viés que o
  método quer corrigir.

---

# AS PROVAS — o que a volta tem de rodar antes de mesclar

**As três primeiras eu já rodei**, no script que imita a regra do roteador
(`detectarGesto`: ordem do array, primeira regex que casa vence, Expressiva só
acima de 120 caracteres). O que a volta tem de fazer é repetir com o app de
verdade, porque **nenhuma volta desta trilha abriu simulador, build ou suíte.**

### 1. O teste da escrita pessoal — **o que não pode falhar de jeito nenhum**

Esta frase, com os 41 no lugar, tem de rotear para a **Expressiva**:

> "na reunião com o chefe eu senti uma raiva enorme, doeu ficar ali, fiquei calado
> o tempo todo e chorei depois no corredor, foi pesado demais para mim"

**Rodei: passa.** A Coluna da esquerda casa nela por duas regex e só não a rouba
porque a Expressiva vem antes no array. Se este teste falhar, **pare a colagem** —
não conserte a regex, conserte a ordem.

### 2. O teste de alcance — nenhum método novo nasce inalcançável
...
**Este é o teste que a M9 quase deixou passar** um método com regex quebrada
(`fizesse\b` não casava "fizessem"). Alcance não é detalhe: é a diferença entre um
método e uma entrada de lista.

### 3. Regressão da fusão — 26 frases antes e depois

**Rodei: uma única mudança fora da Inversão, e é melhora.** A tabela está em 1.3.

### 4. As que só o app pode rodar

- **`swift test` inteiro**, com atenção aos dois de `CatalogoTests` que a fusão
  quebra (§1.5) e ao `osVinteEUmTemProveniencia`, que varre `funcao` de todos.
- **Nenhum encadeamento apontando para id inexistente** — a guarda de dado já
  entrou na M3; ela tem de continuar verde com 41.
- **A Lente de uma nota antiga de Inversão**, mostrando a linha `metodo-ausente`
  com os campos preservados. É a prova de que a §1.6 é verdade na tela, e não só
  no código.
- **O widget e a tela bloqueada**, se o Destaque for tocado por qualquer motivo:
  `Sessao.aplicarDestaque` publica de qualquer método com campo `unica`.
- **A análise de bordo**: os 41 do catálogo contra os **dez** do `GestoDeBordo`.
  Isto **não é prova desta volta** — é o teto que a A2 conserta —, mas quem colar
  precisa saber que o modelo de bordo vence a regex em `Sessao.escolher`, e que
  cada método novo é meio método enquanto a A2 não entrar.

---

# O que este pacote NÃO faz, de propósito

- **Não toca em Swift**, exceto pelas duas linhas de teste do §1.5 e — se o dono
  aceitar o conserto da frase — uma linha em `Gesto.swift` e a string do teste.
- **Não mexe na ordem** de nenhum método já existente. Ordem é comportamento.
- **Não conserta os cinco desvios herdados** (II.1): o `(?m)^quero` do WOOP
  engolindo quatro métodos, e "ideia" da Nota permanente engolindo Destilar. São
  pré-existentes, estão medidos, e têm volta própria no RUMO. **Colar não deve
  tocar neles** — misturar conserto com colagem é como se perde a prova de que a
  colagem não quebrou nada.
- **Não remove nem a Palavra nem o Destaque.** A M14 propôs; o dono decidiu que
  ficam, e o motivo está no RUMO.
- **Não resolve as três superfícies de escolha** (chips por capacidade, folha de
  formas no molde do `MenuFormasView`, e as duas listas fechadas em código). São
  volta de app, e estão no RUMO.

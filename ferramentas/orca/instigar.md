...
| Correção | 9 | **1049 testes em 166 suítes verdes na árvore JÁ MESCLADA com `origin/main`**, no `A1DF082C`, sob `com-trava.sh` — a volta que mede fecha com o binário que mediu. Build LIMPO: **zero aviso e zero erro de compilação**. Foram DUAS mesclas e duas corridas: a primeira deu 1047/165 e o `origin/main` avançou enquanto ela corria, então a segunda refez a conta na árvore de agora. As guardas novas provadas VERMELHAS por mutação: a do contrato com a cláusula do LOTE-5 de volta (3 issues), a do pareamento com UM espaço a mais no braço da base (1 issue), e as duas do carimbo — **apagar a chave da 10b derruba a prova de valor e deixa as outras duas verdes; apagar a linha que copia os carimbos para o registro derruba SÓ a que lê o fonte**, que é exatamente o buraco que ela existe para tapar |

...
**No aparelho da conta (`B91C8DEF`, iPhone 17 Pro teste 2): `ferramentas/orca/d1/d1-notas-15s-conta.mp4`** (15,000 s, 1206×2622, h264 30 fps, sem faixa de áudio, `ffprobe`), gravado às 12h43 de 09/09 com o **caderno real do dono**, no binário desta volta. Autorizado pelo orquestrador (D1-B): a Q2-D tinha encerrado sem instalar, o binário no aparelho era o do `main`. A corrida, sob `com-trava.sh`, sem `erase`, `uninstall` ou `shutdown` (`ferramentas/orca/d1/video-15s.py`, log `video-conta.log` no scratchpad): matei **só** o `serve-sim` do `B91C8DEF` (pid 33936, `kill`), reatei com `attach`, e o helper tocou de verdade (cada passo conferido nos quadros abaixo). **A conta, pela tela do Perfil, antes e depois do install por cima (uma vez, `simctl install`, 12h42m58):** `d1/conta-01-perfil-antes-1242.png` (12h42m52) e `d1/conta-02-perfil-depois-1243.png` (12h43m10) mostram as duas o mesmo "CONTA · Grok · conectada — o Grok é o motor, pago pela sua assinatura." e "Sair da conta — voltar ao motor local"; a árvore de AX leu o mesmo texto nas duas. **A conta não caiu.**

**O que se vê no vídeo do dono, abertos os quadros (`video-conta/q/montagem-conta.png`):** 0–2 s a lista em repouso — "Notas", "Todas ⌄ · Mais recentes ⌄", a frase-porta "trabalhos ›", a cobrança em âmbar "O que aconteceu?" com "Decidir se troco de plano de celular Fico com o atual." embaixo, "setembro", "Quero dormir mais cedo esta semana / WOOP · se eu pegar o celular, então deixo el… / Saúde ⌄", "Plano da semana / ontem / Dinheiro ⌄", "Estou desenhando a tabela de rotas do app / WOOP · tabela pronta / Trabalho ⌄", "conversar com a Ana sobre a viagem de outubro / há 2 dias / Pessoas ⌄", "Dentista quinta às 15h"; 2–4 s a rolagem até "julho" ("Ideia: um caderno que responde · há 40 dias · Ideias ⌄", "juros compostos é dinheiro que trabalha enquanto você dorme · Destilar") e "novembro 2025" ("Mudança para o apartamento novo · há 310 dias · Casa ⌄"), com "Tenho medo de que o servidor caia no dia da apresentação · Pré-mortem · há 20 dias · Trabalho ⌄" no caminho; 4–7 s a volta ao topo; 8–10 s o menu do filtro aberto (Todas ✓, Domínio: Trabalho, Casa, Saúde, Dinheiro, Pessoas, Estudo, Ideias; Método: WOOP, Se–então…); 10–12 s "Saúde" escolhido — "Saúde ⌄ · Mais recentes ⌄", "4 notas · Saúde", e só as quatro de saúde ("Quero dormir mais cedo…", "Dentista quinta às 15h", "Se eu sentir vontade de abrir o celular na cama, então deixo ele…", "Correr três vezes por semana antes do trabalho · WOOP · 10 km sem parar", em "setembro" e "agosto"); 12–15 s o menu de novo, com "Saúde ✓", e "Todas" de volta à lista inteira. Nada de sistema aparece além dos menus do próprio iOS.

**Achado de instrumento, para a ESTEIRA:** `simctl io recordVideo` estica o relógio — o roteiro levou 15,5 s de parede (12h43m16 → 12h43m31) e o arquivo bruto saiu com 21,37 s (668 quadros, taxa variável), nos dois aparelhos (o do teste 3 saiu com 21,29 s). O arquivo entregue é o bruto retimado uniformemente para 15 s (`ffmpeg -vf "setpts=PTS*15/21.37,fps=30"`), sem cortar nenhum quadro — o tempo entregue é o de parede, não o que o `recordVideo` carimbou. E `orca emulator list --json` passou a devolver um objeto único (`result.pid`, `result.device`), não mais `result.streams[]` — o script que mata o helper por lista precisa ler o novo formato.

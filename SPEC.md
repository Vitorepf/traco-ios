...
**A distância.** O G3 da volta Q recusou a 08k por três coisas, e a segunda é esta: manter `produzir`, `prepararPratica` e `revisar` como oferta **contradiz a régua que nós mesmos escrevemos** — timeout material significa operação ausente para o autor. A 08k tinha o dado e mesmo assim deixou a oferta de pé: **20 de 72** chamadas a `grok-4.6` morriam aos 91 s (`prepararPratica` 11 de 18, 61 %), contra **0 de 177** em `grok-4.3`. Anunciar no Perfil "só com a conta Grok: preparar exercícios" quando a operação chega em 1 de 6 casos é prometer o que não se entrega.
...
- **Teto: o único que não invalida a medida de qualidade.** Mesmo modelo, mesmo `reasoning_effort`, mesmo prompt, mesma janela — muda só a paciência. Baixar o raciocínio, que a 08k também listava, mudaria o que foi medido e obrigaria a remedir as dezesseis.

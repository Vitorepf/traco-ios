...
## ADR 2026-09-08k — Quem responde, medido COM a conta: a quarta regra da tabela (volta Q)
...
**A medida.** Sonda `AvaliacaoIA` pelo caminho de produção (`TRACO_AVALIAR_IA`, ADR 07a), 16 operações × 6 casos × 3 execuções em três lançamentos distintos, sem memo, no candidato `325c819`; bundle instalado conferido por sha256 (`Traco` `504d29d7…`, `Traco.debug.dylib` `2152892a…`); fixture `prova/q-qualidade-casos.json` sha256 `29654d46…`; saídas inteiras em `prova/q-qualidade-avaliacoes.jsonl`; leitura em `ferramentas/orca/q-qualidade.md`. `contaGrokLigada: true` em todos os registros, e a corrida de fumaça devolveu a listagem autenticada de modelos. Um caso só passa se as TRÊS execuções cumprirem todos os requisitos obrigatórios; média não aprova nada.
...
**Defeito de disponibilidade, medido e não consertado aqui.** As 12 falhas de transporte caíram todas nas quatro rotas de Trabalho, que pedem `grok-4.6` com raciocínio e teto de 90 s: prepararPratica 11 de 18 (61 %), revisar 6 de 18, produzir 3 de 18 — 20 de 72 chamadas a `grok-4.6`, sempre aos 91 s, e sempre concentradas nos mesmos casos. As rotas em `grok-4.3` tiveram 0 falhas em 177 chamadas. `prepararPratica` fica em **1 de 6 casos** por causa disso, não por conteúdo. Para o autor, uma operação que falha metade das vezes por tempo é uma operação que não está lá. O conserto (teto maior, retentativa, ou raciocínio menor) fica na Q-B; as quatro NÃO são cortadas porque o conteúdo serve quando chega.

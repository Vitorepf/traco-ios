...
**Visto na tela** (17e, 17/09 10:07): apagar «Teste» pelo menu da lista mostra «nota apagada. · Desfazer» no pé; o toque devolve a nota ao mesmo lugar. **Não medido e dívidas.** O véu do seletor de apps e o Face ID pelo código do aparelho estão provados por compilação, não por olho (o simulador não tem biometria). Fluxos maestro atualizados por leitura, não corridos. Ficam: o corte de 9.000 caracteres das candidatas de `ecos` (mora nas linhas da E6b em curso); a citação das Notas conferida por id e não por texto; a `Sessao` repetindo à mão as projeções depois do commit; a mídia de prova que infla o `.git` (1,1 GB) — reescrever a história é decisão do dono.

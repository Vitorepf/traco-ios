...
**Guarda de obra.** Pedido de livro, autor ou tese: se a obra não está nas Fontes que já passaram pelo selo, a base é `insuficiente`, a frase contém «não está no caderno», oferece plantar e não inventa. A recusa é local (`GuardaDeObra`) — o modelo não é chamado. «O que o Autor Sobrenome diz» / «pensa» / «explica» / «argumenta» / «escreve» contam: o nome antes ou depois do verbo é o mesmo pedido. «De acordo com» e «na opinião de» são o mesmo pedido que «segundo». O gesto «Plantar esta obra» grava uma nota com o nome que ela pediu (`Sessao.plantarObra`); origem dela, sem tese inventada. Obra plantada na nota autorizada responde pelas notas. Sem aceite não há cânone. A base `geral` continua para método e conceito sem obra nomeada (WOOP, prazo, conta). Sem auditoria de selo não abre pesquisa (Fase 2).

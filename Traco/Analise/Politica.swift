...
enum Politica {
    ...
    static func linha(_ op: Operacao) -> Linha {
        ...
        case .ecos:
            .init(regra: .indisponivelPorQualidade, porque: "sem retorno 6 de 6 no aparelho; e com a conta ligada em 08/09 o Grok devolveu lista vazia justamente onde o vínculo era o mais útil (18 inscritos contra a sala que comporta 15) — 3 de 6 casos reprovados — ferramentas/orca/q-qualidade.md, corridas em prova/q-qualidade-avaliacoes.jsonl. CONFERIDA em 10/09 sobre a MESMA corrida, e a contagem fecha: o caso do vínculo mais útil volta [] nas 3 repetições, `qn-ecos-nota-curta` também, e `q5-ecos-sentido-e-contradicao` perde o índice 2 em 1 das 3. O que NÃO se pode afirmar é a CAUSA: aquele JSONL é anterior ao `Grok.Diagnostico.bruto` (ADR 10b) e não guarda o retorno do modelo — zero ocorrências no arquivo inteiro —, então lista vazia VINDA do modelo e lista DERRUBADA pela nossa tesoura (`parseEcos` só aceita o trecho como substring literal da candidata, e descarta em silêncio) são indistinguíveis no registro. É NÃO VERIFICÁVEL, não 'nenhuma derrubada'. O indício disponível vai contra a tesoura: nas repetições com vínculo, os 6 trechos passaram literais. Quem remedir com o bruto, nos dois modelos, decide — ferramentas/orca/cauda-ecos-calibragem-recordar.md",
                  motivo: "deixa de fora justamente as notas que mais tinham a ver",
                  medidaEm: "08/09/2026")
        case .calibragem:
            .init(regra: .indisponivelPorQualidade, porque: "vazio 6 de 6 no aparelho. RELIDA em 10/09 sobre a MESMA corrida de 08/09, e as duas metades da razão velha separam-se com número. UMA das 6 morreu na NOSSA porta e nunca chegou ao provedor: `qn-calibragem-par-unico` tem `chamadasGrok` VAZIO nas 3 repetições, porque `Sabia.lerCalibragem` recusa antes de chamar (`guard pares.count >= 2`, 'um par não é padrão'). O autor com um par só nunca teve IA nenhuma, e nenhum conserto de pedido muda isso — isto não é qualidade de modelo. As outras 5 CHEGARAM (conteúdo completo, HTTP 200, nas 15 execuções). Delas, só UMA é qualidade de IA: `qn-calibragem-previsao-acertada` volta [] 3 de 3 onde o requisito exige pergunta — calou sobre o que sustentou os acertos. `q5-calibragem-sem-atraso` também volta [] 3 de 3, e ali o requisito ACEITA vazio: passa. Quem conserta a metade nossa é quem decidir o contrato do par único — o código e o caso da fixture discordam, e a fixture é a mais nova — ferramentas/orca/q-qualidade.md, prova/q-qualidade-avaliacoes.jsonl e ferramentas/orca/cauda-ecos-calibragem-recordar.md",

...
// MARK: - A marca na tela

/// O risco do feriado (dono, 03/set: "um risco pequeno e em diagonal").
///
/// `Capsule` e não `Rectangle`: ponta arredondada é o que separa uma marca
/// desenhada de um traço de debug. E a cor vem de FORA — é sempre a tinta do
/// próprio número, então funciona igual no chip carvão e no papel, sem um
/// único condicional de tema.
///
/// A `largura` é medida no DÍGITO, não na célula: calibrado na tela em 03/set,
/// um risco do tamanho do círculo (24pt sobre 28pt) lia como símbolo de
/// proibido e engolia o número. Curto, ele lê como marca SOBRE o número — que
/// é o que o dono pediu ("um risco pequeno e em diagonal").
nonisolated struct RiscoFeriado: View {
    var largura: CGFloat
    var espessura: CGFloat = 1.2
    var cor: Color

    var body: some View {
        Capsule(style: .continuous)
            .fill(cor.opacity(0.8))
            .frame(width: largura, height: espessura)
            // −40°: num risco curto, a diagonal precisa ser franca. A −32° ele
            // quase deitava e lia como sublinhado torto.
...
nonisolated extension View {
    /// Rasura o número quando o dia não é útil. Some sozinho quando não é.
    func riscoDeFeriado(_ marcado: Bool, largura: CGFloat,
                        espessura: CGFloat = 1.3, cor: Color) -> some View {
        overlay {
            if marcado {
                RiscoFeriado(largura: largura, espessura: espessura, cor: cor)
            }
        }
    }
}

...
**E6c ampliada (líder, 17/09, registrada ANTES de rodar):** `candidatasDoAutor` também escolhe as candidatas da pergunta nas Notas (`comNotasPeloSentido`) — rota ligada, o coração do segundo cérebro. No MESMO caderno sintético de ≥ 200 notas, o agente cego escreve também ≥ 6 perguntas cuja nota esperada é antiga e não divide palavra com a pergunta, mais 2 controles com palavra em comum. **Primeiro sem IA:** a nota esperada está entre as candidatas de `candidatasDoAutor`? Só se faltar desenha-se braço novo; nada muda na rota das Notas sem essa medida.

## E6c — caderno e medida sem IA (registrados ANTES de medir)

**Caderno:** `prova/e6c/caderno.json` (sha `a405fa44…`), SINTÉTICO, escrito por agente cego que não leu arquivo nenhum do repositório: 236 notas de 01/03/2025 a 15/09/2026 (mais densas no fim; 5 longas de 2.000 a 6.000 caracteres), 10 casos de eco (nota recente; 14 esperadas, das quais 13 sem palavra de 4+ letras em comum e 10 com mais de 6 meses) e 8 perguntas das Notas (6 com a esperada antiga e sem palavra em comum; 2 controles com palavras em comum).

**Medida sem IA** (teste que só roda com `TRACO_MEDIR_E6C`, código do app, notas em memória com as datas do caderno), gravada em `prova/e6c/selecao-sem-ia.json`: para cada eco, quais esperadas entram (A) nas 40 recentes de `Sessao.candidatasDeEcos` e (B) em 20 recentes + 20 pelas palavras raras da nota (palavra de 4+ letras em no máximo 1/5 das notas; pontuação só nos 3.000 caracteres que o modelo vê, teto de 40 palavras); para cada pergunta, se a esperada entra nas 30 de `Sessao.candidatasDoAutor`; e o tempo de cada seleção com 400 notas (o caderno repetido). **Leitura prevista pela posição, antes de rodar:** das 14 esperadas dos ecos, só 2 estão entre as 40 mais recentes; as perguntas sem palavra em comum têm a esperada entre as posições 166 e 234 — nenhuma seleção por data ou por palavra as alcança. Se isso se confirmar, nenhum dos braços passa a barra e o vínculo antigo sem palavra pede outro mecanismo (fica para o líder).

**Correção do registro acima:** são **12** esperadas nos 10 ecos (dois casos têm duas), não 14; **11** sem palavra de 4+ letras em comum com a nota e **8** com mais de 6 meses.

**Do líder, recebido enquanto a medida rodava (o resultado abaixo já existia quando li):** (1) o caderno é **adversarial de propósito** — a taxa medida nele NÃO se generaliza ao caderno do dono; a pergunta que ele responde é "achamos o vínculo antigo e sem palavra em comum?", não "quanto acertamos em geral". (2) **Braço C**, desenhado pelo líder sem ver o resultado: a Sábia olha um ÍNDICE do caderno inteiro — uma linha por nota (título, data e a 1ª frase, ou a leitura guardada da E9 quando houver), com teto de caracteres registrado, só notas do autor, sem selada nem expressiva —; numa 1ª chamada o Grok escolhe até 15 candidatas, e a 2ª chamada é o ecos de hoje com o texto delas; o mesmo índice serve à pergunta das Notas. Mede-se no Air: a esperada entre as 15 escolhidas e na resposta final, controles vazios, zero não literal, tempo total e tamanho do pedido. A e B sem IA ficam como linha de base contra C.

## E6c — medida sem IA (`prova/e6c/selecao-sem-ia.json`, teste `SelecaoE6cMedidaTests`, 17e)

- **Ecos:** das 12 esperadas, **A (40 recentes) acha 2** (`c05` n196, 20 dias; `c07` n202, 14 dias) e **B (20 recentes + 20 palavras raras) acha 1** (`c09` n187, pela palavra "cartão" — B perde as duas recentes de A porque só guarda 20 datas). Nenhuma esperada antiga entra em nenhum braço.

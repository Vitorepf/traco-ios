...
struct RevisaoConferenciaNotasTests {
    @Test func obraOmitidaNaoCalaParteApoiadaPorOutraNota() async throws {
        ...
        let cru = #"{"base":"notas","texto":"Esta consulta não trouxe o trecho da obra sobre liderança. Sua anotação reserva 80 reais; falta consultar o preço para comparar ao valor reservado.","trechoIDs":["№1.1"]}"#
    ...
    @Test func referenciaFinalVemDaConferenciaENaoDaCandidata() async throws {
        ...
        let ruim = #"{"base":"notas","texto":"O livro recomenda obedecer a líderes.","trechoIDs":["№1.1"]}"#
        let boa = #"{"base":"notas","texto":"Seu argumento repete a conclusão. Qual evidência independente mostra o benefício do serviço?","trechoIDs":["№2.1"]}"#
        var chamadas = 0
        let r = try #require(await Sabia.responderNasNotas(
            ...
            gerarRemoto: { pacote in
                ...
                #expect(pacote.trechos.last?.id == "№2.1")
    ...
    @Test func conferenciaInvalidaNaoPublicaNemIniciaFallback() async {
        ...
        let r = await Sabia.responderNasNotas(pergunta: "O que falta para comparar a compra ao orçamento?", fontes: [f],
            gerarRemoto: { _ in #"{"base":"notas","texto":"Falta consultar o preço e comparar a 80 reais.","trechoIDs":["№1.1"]}"# },
    ...
    @Test func edicaoSemMudarDataESeloDuranteConferenciaDescartam() async throws {
        for selar in [false, true] {
            ...
            var conferiu = false
            let cru = #"{"base":"notas","texto":"Falta consultar o preço e comparar a 80 reais.","trechoIDs":["№1.1"]}"#
